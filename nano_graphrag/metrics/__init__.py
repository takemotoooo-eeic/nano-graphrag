...
async def compute_answer_correctness(
    ...
    similarity_score = (
        await calculate_semantic_similarity(embeddings, answer, ground_truth)
        if weights[1] != 0
        else 0.0
    )
...
async def calculate_semantic_similarity(
    ...
    a_embed, gt_embed = await asyncio.gather(
        embeddings.aembed_query(answer), embeddings.aembed_query(ground_truth)
    )
