...
def _chunk_context(
    context: str, max_length: int = 21000, chunk_size: int = 3000
) -> List[str]:
    """
...
async def _evaluate_single_context(
    ...
    prompt = CONTEXT_RELEVANCE_JUDGE_BY_EVIDENCE_PROMPT.format(
        context=truncated_context, evidence=evidence_str, question=question
    )
    ...
    for _ in range(max_retries):
        try:
            ...
            parsed = await parser.parse_with_fallbacks(
                response.content, llm=llm if self_healing else None, callbacks=callbacks
            )
...
def _normalize_relevance_response(
    ...
    if isinstance(parsed, dict):
        ...
        if score is not None:
            try:
                return {"reason": str(reason), "relevance_score": int(score)}
